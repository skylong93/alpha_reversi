...
def collect_experience(board, model_black, model_white, episode, experience_queue, experience_size):
    gamma = 0.9
    round_path = list()
    round_r = 10
    for i in range(experience_size):
        ...
        if terminal:
            step = len(round_path)
            for round_idx in range(len(round_path)):
                experience_without_t = round_path[round_idx]
                # 颜色相同，则为正收益，颜色相反，则为负收益
                if playerWin == None:
                    experience_without_t['r'] = 0
                elif experience_without_t['p'] == playerWin.value:
                    experience_without_t['r'] = (gamma ** (step - 1 - round_idx)) * round_r
                else:
                    experience_without_t['r'] = -(gamma ** (step - 1 - round_idx)) * round_r
                experience_queue.append(experience_without_t)
...
def main():
    ...
    for episode in range(episodes):
        ...
        board.reset()
        collect_experience(board, model_black_predict, model_white_predict, episode, experience_queue, queue_size)
        ...
        for idx in range(len(experience_list)):
            ...
            # 500次数据，就做一次训练
            if (idx + 1) % 500 == 0 or idx + 1 == len(experience_list):
