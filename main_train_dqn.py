...
def main():
    ...
    for episode in range(episodes):
        ...
        for i in range(queue_size):
            # 收集数据
            collect_experience(board, model_black, model_white, episode, experience_queue)
        ...
        for idx, experience in np.ndenumerate(np.array(np.random.choice(experience_queue))):
            ...
            if experience["p"] == Player.BLACK.value:
                model_practice = model_black
                reward = experience["r"] if "r" in experience else -9999999
            else:
                model_practice = model_white
                reward = -experience["r"] if "r" in experience else -9999999
        ...
        if len(train_input_black) != 0:
            model_black.fit(np.array(train_input_black), np.array(train_output_black), epochs=5, batch_size=128)
        if len(train_input_white) != 0:
            model_white.fit(np.array(train_input_white), np.array(train_output_white), epochs=5, batch_size=128)
        train_input_black = list()
        train_input_white = list()
        train_output_black = list()
        train_output_white = list()
        model_black.save('model_black.h5')
        model_white.save('model_white.h5')
